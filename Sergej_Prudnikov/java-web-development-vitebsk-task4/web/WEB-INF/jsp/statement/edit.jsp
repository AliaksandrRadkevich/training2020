...
<u:page title="${title}">
	...
	<form action="${urlStatementSave}" method="post">
		...
		<c:if test="${not empty statementByEntrant.userId}">
            <input name="userId" value="${statementByEntrant.userId}" type="hidden">
        </c:if>
		<table>
			<tr>
				<th><h3>
        ...
        <c:if test="${not empty statementByEntrant.passportId}">
            <button class="delete" formaction="${urlStatementDelete}" formmethod="post" ><fmt:message key="statement.edit.button.delete"/></button>
        </c:if>
