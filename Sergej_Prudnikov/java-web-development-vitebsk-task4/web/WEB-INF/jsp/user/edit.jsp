...
<c:if test="${empty user}"><jsp:useBean id="user" class="by.epam.jvd.vitebsk.task4.domain.User"/></c:if>
<c:choose>
...
<u:page title="${title}">
    ...
    <form action="${urlUserSave}" method="post">
        ...
        <label for="email"><fmt:message key="user.edit.form.email"/>:</label>
        ...
        <c:if test="${not empty user.id}">
            <button class="delete" formaction="${urlUserDelete}" formmethod="post"><fmt:message key="user.edit.button.delete"/></button>
        </c:if>
        <c:if test="${not empty user.id}">
